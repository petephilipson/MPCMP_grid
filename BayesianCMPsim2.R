# Code to fit Bayesian MPCMP model for simulation 2
library(mpcmp)
library(mvtnorm)
...
run_sims_2 <- function(Nsims = 1, n = 2000){
  q <- c(0.005, 0.995, 0.025, 0.975, 0.05, 0.95)
  fit1res <- matrix(NA, nrow = Nsims, ncol = 46)
  fit2res <- matrix(NA, nrow = Nsims, ncol = 46)
  fit3res <- matrix(NA, nrow = Nsims, ncol = 46)
  fit1cred <- matrix(NA, nrow = Nsims, ncol = 44*6)
  fit2cred <- matrix(NA, nrow = Nsims, ncol = 44*6)
  fit3cred <- matrix(NA, nrow = Nsims, ncol = 44*6)
  for (i in 1:Nsims){
    print(i)
    dat <- sim2(n = 5000, nref = 50)$data
    #fit1 <- fit_huang(dat, niters = 1000)
    fit2 <- fit_grid_1_sim2(dat, niters = 5000, thin = 1)
    #fit3 <- fit_grid_2(dat)
...
fit_huang <- function(dat, niters = 1000){
  contrasts(dat$ref_id) <- contr.sum(nlevels(dat$ref_id))
  ...
  # Fitted betas
  Beta.hat <- fit.original$coefficients[1:4]
  
  # Fitted referee coefs
  theta.hat <- c(fit.original$coefficients[-(1:4)])
  # Since one is held as reference in glm.cmp fit
  ntheta <- length(theta.hat) 
  
  # Fitted dispersion
  nu.hat <- rep(fit.original$nu, ntheta + 1)
  
  # Covariance matrix for proposal distribution
  p <- length(Beta.hat) #dim(x.matrix)[2]
  Sigma_B <- 0.9*fit.original$variance_beta[1:p, 1:p]
  Sigma_theta <- 0.3*fit.original$variance_beta[-(1:p), -(1:p)]
  ...
  nu0 <- log(nu.hat)
  loglike_0 <- sum(dcomp(y, means0, nu0, log.p = T, summax = 13)) #+ dmvnorm(Beta1, mu0, Sigma0, log=T)
  
  K <- niters
  Betas <- matrix(, nrow = K, ncol = p)
  thetas <- matrix(, nrow = K, ncol = ncol(x.matrix) - p)
  nus <- matrix(, nrow = K, ncol = ntheta + 1)
  colnames(Betas) <-  paste("x", 0:3, sep = "")
  colnames(thetas) <-  paste("ref", 1:length(theta.hat), sep = "")
  colnames(nus) <- paste("dispersion", 1:(length(theta.hat) + 1), sep = "")
  iter_times <- rep(0, K)
  ptm <- proc.time()
  for(k in 1:K){
    print(k)
    ...
    nu0_lp <- exp(nu0[dat$ref_id])
    loglike_1 <- sum(dcomp(y, means1, nu0_lp, log.p = T, summax = 13)) #+ dmvnorm(Beta1, mu0, Sigma0, log=T)
    ...
    # theta update
    theta1 <- rmvnorm(1, theta0, Sigma_theta)[1, ] # rnorm(ntheta, theta0, 0.30) #
    ...
    # Dispersion update
    nu1 <- rnorm(ntheta + 1, nu0, 0.5)  
    nu1_lp <- exp(nu1[dat$ref_id])
    loglike_1 = sum(dcomp(y, means0, nu1_lp, log.p = T, summax = 13))
    nu.ratio = nu1/nu0
    loglikediff = loglike_1 - loglike_0
    logprob = log(runif(1))
    if(logprob < loglikediff){
...
fit_grid_1_sim2 <- function(dat, niters = 1000, thin = 1){
  ## Fit full model
  contrasts(dat$ref_id) <- contr.sum(nlevels(dat$ref_id))
  fit.original <- glm.cmp(y ~ x2 + x3 + x4 + ref_id, dat)
  ...
  # Covariance matrix for proposal distribution
  p <- length(Beta.hat) #dim(x.matrix)[2]
  Sigma_B <- 1.2*fit.original$variance_beta[1:p, 1:p]
  Sigma_theta <- 0.05*fit.original$variance_beta[-(1:p), -(1:p)]
  # Previous code for 20 refs: 0.3*fit.original$variance_beta[-(1:p), -(1:p)]
  # Prefix of 0.1 for 50 refs
  # Prior mean and variance for Beta
  mu0 <- rep(0, p)
  Sigma0 <- diag(p)*10^5
  ...
  for(k in 1:K){
    ...
    nu0_lp <- nu0[dat$ref_id] #exp(nu0[dat$ref_id])
    if (max(mu1) >= 10){mu1 <- pmin(9.9, mu1)}
    if (min(mu1) < 0.001){mu1 <- pmax(0.001, mu1)}
    if (max(nu0_lp) >= 10){nu0_lp <- pmin(9.9, nu0_lp)}
    ...
    lG <- log(rowSums(exp(tcrossprod(log_lambda, unique) - tcrossprod(nu0_lp, lfac)))) 
    lG1 <- log(rowSums(exp(tcrossprod(log_lambda1, unique) - tcrossprod(nu0_lp, lfac)))) 
    # Next line is for component-wise update for theta
    # denrat <- with(dat, tapply(y*(log_lambda1 - log_lambda) + lG - lG1, ref_id, sum))[-(ntheta + 1)]
    ...
    # Dispersion update
    repeat{
    nu1 <- rexp(ntheta + 1, 1/nu0) #rnorm(ntheta + 1, nu0, 0.7) 
    if (max(nu1) <= 10 && min(nu1) > 0.001) break
    }
    #if (max(nu1) >= 10){nu1 <- pmin(9.9, nu1)}
    #if (min(nu1) < 0.001){nu1 <- pmax(0.001, nu1)}
    nu1_lp <- nu1[dat$ref_id] #exp(nu1[dat$ref_id])
    log_lambda <- log(lam_grid_poly_10K[cbind(round(mu0*1000), round(nu0_lp*1000))]) 
    ...
    denrat <- with(dat, tapply(y*(log_lambda1 - log_lambda) + lG - lG1 + 
                                 (nu0_lp - nu1_lp)*lfac_y, ref_id, sum))
                                  + log(nu.ratio) + nu.ratio - 1/nu.ratio
                                  + pexp(nu0, log.p = TRUE) - pexp(nu1, log.p = TRUE)
    laccept <- pmin(0, denrat)
    accept <- (log(runif(ntheta + 1)) < laccept)
    nu0[accept] <- nu1[accept]
    nus[k,] = nu0
    iter_times[k] <- proc.time()[3] - start
  }
  # Get dispersion back on usual scale
...
logZ <- function(log_lambda, nu, summax = 10) {
...
dcomp <- function(x, mu, nu = 1, lambda, log.p = FALSE, lambdalb = 1e-10,
                  lambdaub = 1000, maxlambdaiter = 1e3, tol = 1e-6, summax = 10) {
